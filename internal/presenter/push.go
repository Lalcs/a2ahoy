...
func PrintTaskPushConfigs(w io.Writer, configs []*a2a.TaskPushConfig) error {
	if len(configs) == 0 {
		_, _ = fmt.Fprintln(w, "No push notification configs found.")
		return nil
	}
	...
	for _, cfg := range configs {
		...
		_, _ = fmt.Fprintf(w, "  %s %-20s %s %-40s %s %s\n",
			styledLabel("ID:"), id,
...
func PrintTaskPushConfigDeleted(w io.Writer, taskID, configID string) error {
	_, _ = fmt.Fprintf(w, "%s Push notification config %q deleted from task %q.\n",
