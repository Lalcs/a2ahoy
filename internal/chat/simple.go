...
func RunSimple(ctx context.Context, c client.A2AClient, card *a2a.AgentCard, baseURL string, useJSON bool, initialParts []*a2a.Part) error {
	...
	for {
		...
		if err := runSimpleTurn(ctx, c, state, text, useJSON, initialParts); err != nil {
...
func runSimpleTurn(ctx context.Context, c client.A2AClient, state *State, text string, useJSON bool, extraParts []*a2a.Part) error {
	req := BuildChatRequest(state, text, extraParts...)
