...
func RunSimple(ctx context.Context, c client.A2AClient, card *a2a.AgentCard, baseURL string, useJSON bool) error {
	...
	for {
		...
		if err := runSimpleTurn(ctx, c, state, text, useJSON); err != nil {
...
func runSimpleTurn(ctx context.Context, c client.A2AClient, state *State, text string, useJSON bool) error {
	req := BuildChatRequest(state, text)
